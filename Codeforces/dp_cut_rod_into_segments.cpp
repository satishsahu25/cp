#include <bits/stdc++.h>

using namespace std;

// int solve(int n, int x, int y, int z){
// if(n==0) return 0;
//     if(n<0) return INT_MIN;

//     int first=solve(n-x,x,y,z)+1;
//     int second=solve(n-y,x,y,z)+1;
//     int third=solve(n-z,x,y,z)+1;

//     int ans=max(first,max(second,third));
//     return ans;

// }

// top-down approach

// int solvememo(int n, int x, int y, int z, vector<int> &dp)
// {
//     if (n == 0)
//         return 0;
//     if (n < 0)
//         return INT_MIN;
//         if(dp[n]!=-1) return dp[n];

//     int first = solvememo(n - x, x, y, z,dp) + 1;
//     int second = solvememo(n - y, x, y, z,dp) + 1;
//     int third = solvememo(n - z, x, y, z,dp) + 1;

//     dp[n] = max(first, max(second, third));
//     return dp[n];
// }

// int cutSegments(int n, int x, int y, int z)
// {
//     // Write your code here.
//     if (n == 0)
//         return 0;
//     //we need dp[n] n--length so we need to go n+1 indexing
//     vector<int> dp(n+1, -1);
//     int ans = solvememo(n, x, y, z, dp);
//     if (ans < 0)
//         return 0;
//     else
//         return ans;
// }

// bottom up approach
int solvetab(int n, int x, int y, int z)
{
    vector<int> dp(n + 1,INT_MIN);
    dp[0] = 0;

    for (int i = 1; i <= n; i++)
    {
        if(i-x>=0)dp[i] = max(dp[i], dp[i - x] + 1);
        if(i-y>=0)dp[i] = max(dp[i], dp[i - y] + 1);
        if(i-z>=0)dp[i] = max(dp[i], dp[i - z] + 1);
    }

    return dp[n];
}

int cutSegments(int n, int x, int y, int z)
{
    // Write your code here.
    if (n == 0)
        return 0;
    // we need dp[n] n--length so we need to go n+1 indexing
    int ans = solvetab(n, x, y, z);
    if (ans < 0)
        return 0;
    else
        return ans;
}


int main()
{

    return 0;
}